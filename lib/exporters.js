// Exports & list merging — pure logic extracted from the UI component (audit R10).
// React-free; directly testable with node --test.

export const MAX_SAVED_CAMPAIGNS = 20;

/**
 * Merges two campaign lists: newest version wins per id, sorted descending,
 * with the maximum limit applied.
 */
...
/** Converts a validated campaign into a readable Markdown document. */
...
// Avoid bare square brackets in Obsidian-bound texts (would break links).
const esc = (s) => String(s ?? "").replace(/\[(?!\[)/g, "");

/**
 * Obsidian export (Phase 7): Markdown with YAML frontmatter, inline tags
 * (#campaign, #mission, #npc, #location) and internal [[NPC Name]] links —
 * known NPC names are auto-linked inside mission text too.
 */
export function campaignToObsidian(c) {
  ...
  // Auto-link exact NPC mentions inside free text.
  const link = (text) => {
  ...
  md += `# ${c.title}\n\n`;
  md += `> **Place:** ${c.setting} · **Tone:** ${c.tone}\n\n`;
  md += `## Premise #premise\n\n${link(c.premise)}\n\n`;
...
/**
 * Foundry VTT export (Phase 7): an "Adventure"-shaped document used by the
 * common import modules — journal entries for premise/missions/NPCs/endings
 * plus actor sheets for each NPC. Content-level compatible.
 */
export function campaignToFoundryVTT(c, characterName = "") {
  ...
  if (c.endings && c.endings.length > 0) {
    journals.push(journal(`${c.title} — Future threads`, `<h2>Future Threads</h2><ul>${c.endings.map((e) => `<li>${esc(e)}</li>`).join("")}</ul>`));
  }
  ...
  const actors = (c.npcs || []).map((n) => ({
    ...
    system: {
      details: {
        biography: `${n.personality || ""}${n.connection || n.connection_to_lara || n.connection_to_character ? `\n\nRelationship with ${characterName || "the protagonist"}: ${n.connection || n.connection_to_lara || n.connection_to_character}` : ""}`,
      },
    },
  }));
...
/**
 * Print-optimised HTML document (Phase 7): medieval fonts, parchment background,
 * clean pagination. Opened in a new window followed by print() → the user picks
 * "Save as PDF".
 */
export function campaignToPrintHtml(c, characterName = "") {
  ...
  const locations = (c.locations || [])
    .map((l) => `<section class="card"><h3>${esc(l.name)} <span class="diff">(${esc(l.vibe || "setting")})</span></h3><p>${esc(l.description)}</p></section>`)
    .join("");
...
<div class="sheet">
  ...
  ${(c.endings && c.endings.length) ? `<h2>Future Threads</h2><ul>${c.endings.map((e) => `<li>${esc(e)}</li>`).join("")}</ul>` : ""}
